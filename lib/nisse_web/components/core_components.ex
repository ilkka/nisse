defmodule NisseWeb.CoreComponents do
  @moduledoc """
  Core components for Nisse UI based on Tailwind CSS.
  """
  ...
  JS commands can be passed to the `:on_confirm` and `:on_cancel` attributes
  for the caller to react to each button press, for example:

    <.modal id="confirm" on_confirm={JS.push("delete")} on_cancel={JS.navigate(~p"/post")}>
      Are you sure?
      <:confirm>OK</:confirm>
      <:cancel>Cancel</:cancel>
    </.modal>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(NisseWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(NisseWeb.Gettext, "errors", msg, opts)
    end
  end
